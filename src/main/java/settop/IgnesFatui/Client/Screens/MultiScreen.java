...
import settop.IgnesFatui.Client.Screens.Popups.ScreenPopup;
import settop.IgnesFatui.Client.Screens.SubScreens.SubScreen;
import settop.IgnesFatui.GUI.MultiScreenContainer;
import settop.IgnesFatui.GUI.Network.Packets.CScrollWindowPacket;
...
public abstract class MultiScreen<T extends MultiScreenContainer> extends ContainerScreen<T>
{
    ...
    protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY)
    {
        for(SubScreen subScreen : subScreens)
        {
            ...
            {
                subScreen.drawGuiContainerBackgroundLayer(matrixStack, partialTicks, mouseX, mouseY);
            }
        }
    }
    ...
    protected void drawGuiContainerForegroundLayer(MatrixStack matrixStack, int x, int y)
    {
        for(SubScreen subScreen : subScreens)
        {
            if(subScreen != null && subScreen.GetSubContainer().IsActive())
            {
                subScreen.drawGuiContainerForegroundLayer(matrixStack, x, y);
            }
        }
    }
    ...
    public static void RenderBorder(AbstractGui gui, MatrixStack matrixStack, int x, int y, int blitOffset, int width, int height)
    {
    ...
    public static void RenderSlotRowBackground(AbstractGui gui, MatrixStack matrixStack, int x, int y, int blitOffset, int numSlots)
    {
        int oldBlitOffset = gui.getBlitOffset();
        gui.setBlitOffset(blitOffset);
        Minecraft.getInstance().getTextureManager().bindTexture(GUI_PARTS_TEXTURE);
    ...
    public void RenderPlayerInv(MatrixStack matrixStack, int playerInvXPos, int playerInvYPos)
    {
        this.minecraft.getTextureManager().bindTexture(GUI_PARTS_TEXTURE);

        int playerInvXOffset = -3;
        int playerInvYOffset = -4;
        this.blit(matrixStack, guiLeft + playerInvXPos + playerInvXOffset, guiTop + playerInvYPos + playerInvYOffset, PLAYER_INVENTORY.uStart, PLAYER_INVENTORY.vStart, PLAYER_INVENTORY.width, PLAYER_INVENTORY.height);
    }
    ...
    {
        int slotID = hoveredSlot == null ? -1 : hoveredSlot.slotNumber;
        if(getContainer().mouseScrolled(slotID, mouseX, mouseY, delta))
        {
            IgnesFatui.MULTI_SCREEN_CHANNEL.sendToServer( new CScrollWindowPacket(getContainer().windowId, slotID, (float)delta));
            return true;
        }
